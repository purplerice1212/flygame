...
- **規則衝突矩陣 / Rule conflict matrix**：詳見 [`RULE_CONFLICT_MATRIX.md`](RULE_CONFLICT_MATRIX.md)，記錄目前實作中的優先序與衝突結果。
...
## Project Finish Roadmap（專案收尾路線圖）

若目標是把目前 MVP 從「可玩原型」推進到「可交付版本」，建議分 5 個里程碑處理：

1. **Stability / 穩定性**
   - 把 `GameRules` 與 `App` 拆成獨立模組（`/src/rules`, `/src/app`），降低單檔維護成本。
   - 補單元測試（至少覆蓋：起飛、吃子、堵路、特殊格順序、跳過回合）。
   - 建立 smoke test（開局→擲骰→移動→存檔→讀檔）。

2. **Game Completeness / 遊戲完整度**
   - 補齊「勝利後流程」：結算頁、排行榜、再次開局。
   - 檢查並統一所有規則衝突優先序（例如：連擲 vs 陷阱跳過）。
   - 新增 3–5 組「規則範本」以方便一般玩家直接開局。

3. **UX & Accessibility / 體驗與無障礙**
   - 針對手機斷點優化控制區（擲骰、可行動列表、戰報）。
   - 補齊鍵盤焦點順序與螢幕閱讀器描述（尤其是棋子可行動狀態）。
   - 加入新手引導（首次進入時一步步教學）。

4. **Persistence & Ops / 存檔與維運**
   - 將存檔升級策略文件化（版本遷移對照表）。
   - 新增匯出/匯入 JSON 存檔，避免只依賴 localStorage。
   - 補上錯誤回報欄位（瀏覽器版本、規則配置、最近 10 筆戰報）。

5. **Release / 發佈**
   - 建立 CI（lint + test + build 靜態頁面檢查）。
   - 撰寫 `CHANGELOG.md` 與 `LICENSE`。
   - 發佈到 GitHub Pages / Netlify 並設定版本標記。

## Definition of Done（完成定義）

專案可視為「完成」的最低門檻：

- 規則引擎測試覆蓋核心流程，且 CI 全綠。
- 2–4 人局至少完成 20 場手動回歸，無阻斷級 bug。
- 手機與桌機都能順暢操作（擲骰、選棋、Undo、存讀檔）。
- README 含部署方式、版本資訊、已知限制與回報流程。
- 有正式授權條款與發佈紀錄。

## Suggested Next 3 Tasks（建議下一步）

如果你要立刻推進，建議先做這三件：

1. 把 `GameRules` 抽成模組並加測試（這一步能最快提升可維護性）。
2. 定義一份「規則衝突矩陣」並逐項驗證目前實作。
3. 上線一個公開試玩連結，收集前 10 位玩家回饋後再進入美術與音效優化。
